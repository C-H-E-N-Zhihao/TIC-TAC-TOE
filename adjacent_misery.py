...
def set_board_up(stones_per_player = (BSIZ**2)//2):
...
#init board and game data here
	...
	#omplir la llista Ls BSIZ**2 pedres de color gris a l'inici
	for x in range(BSIZ):
		for y in range(BSIZ):
			if ((x,y,PLAYER_COLOR[1]) not in Ls) and ((x,y,PLAYER_COLOR[0]) not in Ls):
				Ls.append((x,y,GRAY))
	...
	def select_st(i,j):
		...
		if stones_per_player == 0: 					#quan arribem a 8 pedres al tauler 
			if tuple(sel_PLAYER) in L:				#si esta en L, aleshores en Ls també
				if ((i+1,j,GRAY)in Ls) or ((i-1,j,GRAY)in Ls) or ((i,j+1,GRAY)in Ls) or ((i,j-1,GRAY) in Ls):
					Ls.remove(tuple(sel_PLAYER))
					Ls.append((i,j,GRAY))
					print("\nStone Selected: ",i,j)
					return True
				else:
					print("\nYou cannot move that stone, there is not any adjacent spot to that stone. Select a new stone.")
					return False
			else: 
				print("\nERROR: Invalid position selected or the stone you want to select is not yours. ")
				return False
		return True		#retorna True sempre i quan no hem posat totes les pedres
...
#Check Diagonally
	...
	def move_st(i,j):
		...
		the end of the game.
		'''
		nonlocal curr_player, select_st, L, Ls, stones_per_player
#casos desfavorables
	#si coordenades equivocades	
	
		if i < 0 or i > (BSIZ-1) or j < 0 or j > (BSIZ-1):
			print("ERROR: Maybe the input's format was wrong.\n")
...
#casos favorables 
	...
	#Player 0
		if curr_player == 0:			
			...
			if stones_per_player != 0:
				stones_per_player -= 1/2
	...
	#quan ja hi ha 8 pedres al tauler
		if stones_per_player ==0:
			k=0
			while k <len(Ls):
				#veure si hi ha espai buit adjacent a les pedres de Ls
				a = ((Ls[k][0])+1,(Ls[k][1]),GRAY) 
				b = ((Ls[k][0])-1,(Ls[k][1]),GRAY)
				c = ((Ls[k][0]),(Ls[k][1])+1,GRAY)
				d = ((Ls[k][0]),(Ls[k][1])-1,GRAY)
					#si hi és i es el color del current player, permet seleccionar
				if (a in Ls) or (b in Ls) or (c in Ls) or (d in Ls): # hay almenos uno 
					if Ls[k][2]==PLAYER_COLOR[curr_player]:
						select_st=False							#funció select es fals
						if curr_player==0:
							print("\nBlue/X Select a stone.")				#escriure al terminal
						else:
							print("\nRed/O Select a stone.")
						return (select_st, curr_player, end())
				k+=1
				#sino, perd el que no té pedres adjacents a l'espai buit
			if curr_player==0:
				print("There is no adjacent places to move for BLUE/X player(right, left, up and down)")
			else:
				print("There is no adjacent places to move for RED/O player(right, left, up and down)")
					
			return(select_st, curr_player, not end()) 
